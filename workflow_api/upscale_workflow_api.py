...
def process_prompt(workflow_api_json: str, 
    ...
    for key, value in data.items():
        if "inputs" in value:
            if "_meta" in value and value["_meta"].get("title") == "Load Input Image":
                input_image_node = value
            elif "_meta" in value and value["_meta"].get("title") == "Largest Side Length (px)":
                resize_image_node = value
